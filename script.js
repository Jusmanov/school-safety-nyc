...
function nycApi(geo) {
	for (var i = 0; i < geo.length; i++){
		...
		crimesTotal.push(Number(geo[i].major_n) + Number(geo[i].oth_n) + Number(geo[i].nocrim_n))
		schoolName = geo[i].location_name;
		comprehensive.push([geo[i].latitude, geo[i].longitude, (Number(geo[i].major_n) + Number(geo[i].oth_n) + Number(geo[i].nocrim_n)), geo[i].location_name])
	}
